...
Tipe data boolean merepresentasikan salah satu dari dua nilai: _true_ atau _false_. Nilai boolean itu true atau false. Nanti bakal makin jelas pas kamu mulai pake operator perbandingan. Setiap perbandingan ngembaliin nilai boolean yaitu true atau false.
...
Coba deh pahami perbandingan di atas dengan sedikit logika. Ngapalin doang tanpa logika mungkin susah.
JavaScript bisa dibilang bahasa pemrograman yang agak aneh sih. Kode JavaScript jalan dan ngasih kamu hasil, tapi kecuali kamu udah mahir, hasilnya mungkin nggak sesuai harapan.

Sebagai aturan praktis, kalau suatu nilai nggak true dengan == maka nggak bakal sama dengan ===. Pake === lebih aman daripada pake ==. [Tautan](https://dorey.github.io/JavaScript-Equality-Table/) berikut punya daftar lengkap perbandingan tipe data.

### Operator Logika

Simbol berikut adalah operator logika yang umum:
&& (ampersand), || (pipe) dan ! (negasi).
Operator && ngasilin true cuma kalau kedua operand bernilai true.
Operator || ngasilin true kalau salah satu operand bernilai true.
Operator ! meniadakan true jadi false dan false jadi true.
...
Operator ternary memungkinkan kita buat nulis kondisi.
Cara lain buat nulis kondisional adalah pake operator ternary. Lihat contoh berikut nih:
...
Gue saranin kamu buat baca tentang prioritas operator dari [tautan](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Operator_Precedence) ini.
...
Metode confirm() nampilin kotak dialog dengan pesan tertentu, barengan sama tombol OK dan Cancel.
Kotak konfirmasi sering dipake buat minta izin dari pengguna buat ngejalanin sesuatu. Window confirm() nerima string sebagai argumen.
Klik OK ngasilin nilai true, sedangkan klik tombol Cancel ngasilin nilai false.
...
Yuk kita format nilai-nilai ini ke format waktu yang bisa dibaca manusia.
**Contoh:**
...
1. Deklarasikan variabel firstName, lastName, country, city, age, isMarried, year dan kasih nilai padanya, terus pake operator typeof buat ngecek tipe data yang beda-beda.
2. Cek apakah tipe dari '10' sama dengan 10
3. Cek apakah parseInt('9.8') sama dengan 10
4. Nilai boolean adalah true atau false.
   1. Tulis tiga pernyataan JavaScript yang ngasilin nilai truthy.
   2. Tulis tiga pernyataan JavaScript yang ngasilin nilai falsy.

5. Cari tahu hasil dari ekspresi perbandingan berikut duluan tanpa pake console.log(). Setelah kamu mutusin hasilnya, konfirmasiin pake console.log()
   1. 4 > 3
   2. 4 >= 3
   3. 4 < 3
   4. 4 <= 3
   ...
   10. 4 == '4'
   11. 4 === '4'
   12. Temukan panjang python dan jargon dan bikin pernyataan perbandingan falsy.

6. Cari tahu hasil dari ekspresi berikut duluan tanpa pake console.log(). Setelah kamu mutusin hasilnya, konfirmasiin pake console.log()
   1. 4 > 3 && 10 < 12
   2. 4 > 3 && 10 > 12
   3. 4 > 3 || 10 < 12
   4. 4 > 3 || 10 > 12
...
7. Pake Date object buat ngelakuin aktivitas berikut
   ...
   7. Temuin jumlah detik yang udah berlalu dari 1 Januari 1970 hingga sekarang.

### Latihan: Level 2

1. Tulis script yang minta pengguna masukin alas dan tinggi segitiga dan ngitung luas segitiga (luas = 0.5 x a x t).
...
1. Tulis script yang minta pengguna masukin sisi a, sisi b, dan sisi c segitiga dan ngitung keliling segitiga (keliling = a + b + c)
...
1. Dapetin panjang dan lebar pake prompt dan hitung luas persegi panjang (luas = panjang x lebar) dan keliling persegi panjang (keliling = 2 x (panjang + lebar))
1. Dapetin jari-jari pake prompt dan hitung luas lingkaran (luas = pi x r x r) dan keliling lingkaran (k = 2 x pi x r) di mana pi = 3.14.
1. Hitung kemiringan (slope), perpotongan sumbu x (x-intercept) dan perpotongan sumbu y (y-intercept) dari y = 2x -2
1. Kemiringan adalah m = (y<sub>2</sub>-y<sub>1</sub>)/(x<sub>2</sub>-x<sub>1</sub>). Temuin kemiringan antara titik (2, 2) dan titik (6,10)
1. Bandingin kemiringan dari dua pertanyaan di atas.
1. Hitung nilai y (y = x<sup>2</sup> + 6x + 9). Coba pake nilai x yang beda-beda dan cari tahu di nilai x berapa y adalah 0.
1. Tulis script yang minta pengguna masukin jam dan tarif per jam. Hitung gaji orang tersebut?
...
1. Kalau panjang nama kamu lebih dari 7, bilang nama kamu panjang, kalau nggak bilang nama kamu pendek.
1. Bandingin panjang nama depan dan nama keluarga kamu, dan kamu harus dapetin output ini.
...
1. Deklarasikan dua variabel _myAge_ dan _yourAge_ dan kasih nilai awal myAge dan yourAge.
...
1. Pake prompt dapetin tahun lahir pengguna dan kalau pengguna berusia 18 tahun ke atas, izinin pengguna buat nyetir, kalau nggak kasih tahu pengguna buat nunggu sejumlah tahun tertentu.
...
1. Tulis script yang minta pengguna masukin jumlah tahun. Hitung jumlah detik yang bisa dijalani seseorang. Asumsikan seseorang hidup cuma seratus tahun.
...
1. Bikin format waktu yang bisa dibaca manusia pake Date time object
   1. YYYY-MM-DD HH:mm
   2. DD-MM-YYYY HH:mm
   3. DD/MM/YYYY HH:mm
...
1. Bikin format waktu yang bisa dibaca manusia pake Date time object. Jam dan menit harus selalu dua digit (7 jam harus 07 dan 5 menit harus 05)
